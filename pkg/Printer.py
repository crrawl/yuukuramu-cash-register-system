...
class Printer:
    ...
    try:
        # If basket.json is not empty then basket = list of products
        if not os.stat(path_to_save_basket).st_size==0:

            basket = Storage.show_basket(1)
        else:
            # If basket is empty than basket = empty basket
            basket = []

    except FileNotFoundError:

        # Creating a basket.json if that not exists
        with open(path_to_save_basket, "w") as f:
            f.close()
        print("Creating a basket...")
        exit()
